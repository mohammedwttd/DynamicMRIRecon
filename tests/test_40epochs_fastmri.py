#!/usr/bin/env python3
"""
Test script to evaluate specific models at 40 epochs on FastMRI test set.

Models: Unet, SmallUnet, RigL40, RigL60, LeanUnet128, LeanUnet64, LeanUnet32

Usage:
    python tests/test_40epochs_fastmri.py --data-path ../data --device cuda
"""
...
def build_model(args, device):
    ...
    model = Subsampling_Model(
        ...
        epochs=getattr(args, 'num_epochs', 40),
...
# Define models to test - all 40-epoch models with best_model.pt
MODELS_TO_TEST = OrderedDict([
    ('Unet', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_Unet_40',
        'description': 'Standard UNet with 32 base channels (40 epochs)',
    }),
    ('SmallUnet', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_SmallUnet_40',
        'description': 'Small UNet with 20 base channels (40 epochs)',
    }),
    ('RigL20', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL20_40',
        'description': 'UNet with RigL at 20% sparsity (40 epochs)',
    }),
    ('RigL40', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL40_40',
        'description': 'UNet with RigL at 40% sparsity (40 epochs)',
    }),
    ('RigL60', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL60_40',
        'description': 'UNet with RigL at 60% sparsity (40 epochs)',
    }),
    ('RigL75', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL75_40',
        'description': 'UNet with RigL at 75% sparsity (40 epochs)',
    }),
    ('RigL80', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL80_40',
        'description': 'UNet with RigL at 80% sparsity (40 epochs)',
    }),
    ('RigL85', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL85_40',
        'description': 'UNet with RigL at 85% sparsity (40 epochs)',
    }),
    ('RigL90', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_RigL90_40',
        'description': 'UNet with RigL at 90% sparsity (40 epochs)',
    }),
    ('LeanUnet128', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0005_fixed_LeanUnet128_40',
        'description': 'LeanUnet with 128 constant channels (40 epochs)',
    }),
    ('LeanUnet64', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.0008_fixed_LeanUnet64_40',
        'description': 'LeanUnet with 64 constant channels (40 epochs)',
    }),
    ('LeanUnet32', {
        'checkpoint': 'summary/16/cartesian_1_4_0.08_0.001_fixed_LeanUnet32_40',
        'description': 'LeanUnet with 32 constant channels (40 epochs)',
    }),
])
...
def main():
    parser = argparse.ArgumentParser(description='Test 40-epoch models on FastMRI test set')
    ...
    parser.add_argument('--max-samples', type=int, default=-1,
                        help='Maximum number of samples to evaluate (default: -1 for all)')
    ...
    parser.add_argument('--m4raw', action='store_true', default=False,
                        help='Evaluate on M4Raw as well')
    parser.add_argument('--m4raw-only', action='store_true', default=False,
                        help='Only evaluate on M4Raw (skip FastMRI brain test)')
    ...
    parser.add_argument('--m4raw-acquisition', type=str, default=None,
                        help='Filter M4Raw by acquisition substring (e.g., T1, T2, FLAIR)')
    ...
    print(f"\n{'=' * 80}")
    if args.m4raw_only:
        print("40-EPOCH MODELS EVALUATION ON M4RAW")
    elif args.m4raw:
        print("40-EPOCH MODELS EVALUATION ON FASTMRI TEST SET + M4RAW")
    else:
        print("40-EPOCH MODELS EVALUATION ON FASTMRI TEST SET")
    print(f"{'=' * 80}")
    print(f"\nUsing device: {device}")
    if device.type == 'cuda':
    ...
    if not args.m4raw_only:
        ...
        for model_name, model_info in MODELS_TO_TEST.items():
            ...
            try:
                ...
                results['fastmri_brain'][model_name] = metrics

                print(f"\nResults for {model_name} (FastMRI Brain):")
                ...
                print(f"\nError testing {model_name} on FastMRI Brain: {e}")
                import traceback
                traceback.print_exc()
                continue
    ...
    if args.m4raw or args.m4raw_only:
        m4raw_max = args.m4raw_max_samples if args.m4raw_max_samples > 0 else None
        print(f"\n{'=' * 80}")
        print("M4RAW EVALUATION")
        print(f"{'=' * 80}")
        ...
        if m4raw_loader is None or m4raw_samples == 0:
            ...
            for model_name, model_info in MODELS_TO_TEST.items():
                ...
                try:
                    ...
                    metrics['sparsity'] = sparsity
                    metrics['masks_applied'] = masks_applied
                    results['m4raw'][model_name] = metrics

                    print(f"\nResults for {model_name} (M4Raw):")
                    print(f"  PSNR: {metrics['psnr_mean']:.2f} ± {metrics['psnr_std']:.2f} dB")
                    print(f"  SSIM: {metrics['ssim_mean']:.4f} ± {metrics['ssim_std']:.4f}")
                    print(f"  Eval time: {metrics['eval_time']:.1f}s")
    ...
    if isinstance(results, dict) and results:
        if 'fastmri_brain' in results and results['fastmri_brain']:
            print_results_table(results['fastmri_brain'], "FASTMRI BRAIN RESULTS (40 EPOCHS, BEST MODEL)")
        if 'm4raw' in results and results['m4raw']:
            print_results_table(results['m4raw'], "M4RAW RESULTS (40 EPOCHS, BEST MODEL)")
