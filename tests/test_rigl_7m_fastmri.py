#!/usr/bin/env python3
"""
Test script to evaluate big RigL models (10-70) with 7M params on FastMRI and M4Raw.
These are models trained with 48 base channels (~7.75M total params).

Usage:
    python tests/test_rigl_7m_fastmri.py --data-path ../data --device cuda
    python tests/test_rigl_7m_fastmri.py --data-path ../data --device cuda --m4raw --m4raw-max-samples 1500
"""
...
def evaluate_on_dataset(models_to_test, data_loader, device, dataset_name="Test",
                        single_shot=False, single_shot_steps=20, single_shot_lr=1e-4):
    ...
    for model_name, model_info in models_to_test.items():
        ...
        if single_shot:
            print(f"Mode: SINGLE-SHOT ADAPTATION (steps={single_shot_steps}, lr={single_shot_lr})")
        print(f"{'=' * 80}")
        ...
        try:
            ...
            if single_shot and masks_applied > 0:
                print(f"Single-shot adaptation on {dataset_name}...")
                ...
                if masks:
                    metrics = single_shot_evaluate(
                        model, data_loader, device, masks,
                        criterion=torch.nn.L1Loss(),
                        model_name=model_name,
                        num_steps=single_shot_steps,
                        lr=single_shot_lr,
                        dataset_name=dataset_name
                    )
                    print(f"  Loss before adaptation: {metrics['loss_before']:.6f}")
                    print(f"  Loss after adaptation:  {metrics['loss_after']:.6f}")
                else:
                    print("  Warning: No masks found, falling back to standard evaluation")
            ...
            if metrics.get('single_shot'):
                print(f"  Single-shot: YES ({metrics['num_steps']} steps)")
...
def main():
    parser = argparse.ArgumentParser(description='Test big RigL models (7M params) on FastMRI and M4Raw')
    ...
    parser.add_argument('--max-samples', type=int, default=-1,
                        help='Maximum number of FastMRI samples to evaluate (default: -1 for all)')
    ...
    print(f"\n{'=' * 80}")
    if args.m4raw_only:
        print("BIG RIGL MODELS (7M PARAMS) EVALUATION ON M4RAW")
    elif args.m4raw:
        print("BIG RIGL MODELS (7M PARAMS) EVALUATION ON FASTMRI + M4RAW")
    else:
        print("BIG RIGL MODELS (7M PARAMS) EVALUATION ON FASTMRI TEST SET")
    print(f"{'=' * 80}")
    print(f"\nUsing device: {device}")
    if device.type == 'cuda':
    ...
    if args.single_shot:
        print(f"\n📊 Single-shot adaptation ENABLED (steps={args.single_shot_steps}, lr={args.single_shot_lr})")
    ...
    if not args.m4raw_only:
        ...
        fastmri_results = evaluate_on_dataset(
            MODELS_TO_TEST, test_loader, device, "FastMRI Test",
            single_shot=args.single_shot,
            single_shot_steps=args.single_shot_steps,
            single_shot_lr=args.single_shot_lr
        )
        all_results['fastmri'] = fastmri_results
        
        if fastmri_results:
            print_results_table(fastmri_results, "BIG RIGL MODELS (7M PARAMS) - FASTMRI TEST RESULTS")
    ...
    if args.m4raw or args.m4raw_only:
        ...
        if m4raw_loader is not None and m4raw_samples > 0:
            ...
            m4raw_results = evaluate_on_dataset(
                MODELS_TO_TEST, m4raw_loader, device, "M4Raw",
                single_shot=args.single_shot,
                single_shot_steps=args.single_shot_steps,
                single_shot_lr=args.single_shot_lr
            )
            ...
            if m4raw_results:
                title = "BIG RIGL MODELS (7M PARAMS) - M4RAW RESULTS"
                if args.m4raw_acquisition:
